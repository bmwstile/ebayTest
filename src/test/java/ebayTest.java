import io.github.bonigarcia.wdm.ChromeDriverManager;
import org.junit.*;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.chrome.ChromeDriver;
import ru.yandex.qatools.allure.annotations.Step;
import ru.yandex.qatools.allure.annotations.Title;

import java.util.concurrent.TimeUnit;

@Title("Тестовое задание")
public class ebayTest {

    private WebDriver driver;

    @Step("Открытие ссылок")
    private void openPage(String s) {
        OpenPage openPage = new OpenPage(driver);
        openPage.checkLink(s);
    }

    @Step("Нажать на кнопку")
    private void clickOnButton(String s) {
        OpenPage openPage = new OpenPage(driver);
        openPage.button(s);
    }

    @Step("Логин")
    private void loginPage(String login, String password) throws InterruptedException {
        LoginPage loginPage = new LoginPage(driver);
        loginPage.login(login, password);
    }

    @Step("Поиск")
    private void search(String x) throws InterruptedException {
        Search search = new Search(driver);
        search.search(x);
    }

    @Step("Проверка")
    private void check() throws InterruptedException {
        Check check = new Check(driver);
        check.check();

    }

    @Step("Логаут")
    private void logout() throws InterruptedException {
        LoginPage loginPage = new LoginPage(driver);
        loginPage.logout();
    }

    @Step("Логаут")
    private void checkLogout() throws InterruptedException {
        Check check = new Check(driver);
        check.logout();
    }


    @Before
    public void start() {
        ChromeDriverManager.getInstance().setup();
        driver = new ChromeDriver();
        driver.manage().timeouts().implicitlyWait(50, TimeUnit.SECONDS);
        driver.manage().window().maximize();
    }

    @After
    public void exit() {
        driver.quit();
    }

    @Test
    @Title("Ebay")
    public void testEbayTask() throws InterruptedException {
        driver.get("https://www.ebay.com/");
        openPage("Войдите");
        loginPage("ivanov-417238","Test1234567890");
        clickOnButton("Вход в систему");
        Thread.sleep(3000);
        search("samsung");
        Thread.sleep(3000);
        check();
        logout();
        Thread.sleep(3000);
        openPage("Выход");
        checkLogout();



    }


}